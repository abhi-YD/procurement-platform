...
const LocationMap = dynamic(() => import("@/components/map/LocationMap"), {
  ssr: false,
  loading: () => (
    <div className="h-[220px] w-full bg-neutral-100 rounded-lg animate-pulse flex items-center justify-center text-xs text-gray-500 font-medium">
...
export default function DeliveryDestination({
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl p-5 text-left space-y-4">
      ...
      <form onSubmit={handleSetLocation} className="flex gap-2">
        <div className="relative flex-1 flex items-center">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-3 border border-neutral-200 rounded-lg text-sm text-[#111827] bg-white outline-none focus:border-[#0F1E3C] focus:ring-1 focus:ring-[#0F1E3C] placeholder:text-neutral-400 font-medium"
          />
        </div>
        <button
          type="submit"
          disabled={searching || !addressInput.trim()}
          className="h-10 px-4 bg-[#0F1E3C] hover:bg-[#1a2f5e] text-xs font-semibold text-white rounded-lg transition-colors disabled:opacity-50 cursor-pointer"
        >
          {searching ?"..." :"Set"}
        </button>
      </form>
      ...
      {coords && destination && (
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-50 border border-green-200 text-xs font-medium text-green-800 animate-fade-in">
          <span>📍</span> {destination}
        </div>
      )}

      {/* Map display - 220px height */}
      <div className="h-[220px] w-full overflow-hidden rounded-lg border border-gray-100 relative z-0 bg-neutral-50">
        <LocationMap 
          value={coords} 
          onPick={(p) => {
            setCoords(p);
